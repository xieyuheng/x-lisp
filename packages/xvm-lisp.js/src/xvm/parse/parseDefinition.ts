...
export function parseDefinition(sexp: S.Sexp): Xvm.Definition {
  ...
  switch (head) {
    case "define-function": {
      ...
      return Xvm.FunctionDefinition(name, parameters, instrs)
    }

    case "declare-variable": {
      const name = S.asSymbolSexp(elements[1]).content
      return Xvm.VariableDeclaration(name)
    }

    case "declare-primitive-function": {
      const name = S.asSymbolSexp(elements[1]).content
      return Xvm.PrimitiveFunctionDeclaration(name)
    }

    case "declare-primitive-variable": {
      const name = S.asSymbolSexp(elements[1]).content
      return Xvm.PrimitiveVariableDeclaration(name)
    }
