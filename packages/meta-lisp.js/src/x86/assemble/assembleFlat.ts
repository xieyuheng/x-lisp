...
import { emptyEnv, evaluate, envPutMany } from "../evaluate/index.ts"
...
export function assembleFlat(mod: Mod): Uint8Array {
  ...
  const totalSize = codeSize + dataLayouts.reduce((s, d) => s + d.bytes.length, 0)
...
function emitFlatValue(
  ...
  if (value.kind === "StructValue") {
    const fieldTypes = dataTypeUnfold(mod, type as DataType, S.zeroLocation("data"))
    let pos = offset
    for (const [fieldName, fieldType] of fieldTypes) {
      const fieldValue = value.fields.get(fieldName)
      if (!fieldValue) throw new Error(`missing struct field: ${fieldName}`)
...
function recordSubfieldLabels(
  ...
  for (const field of structFields) {
    ...
    if (fieldType.kind === "DataType") {
      ...
      if (subDef && subDef.kind === "StructDefinition" && subDef.fields.length > 0) {
        recordSubfieldLabels(
          mod,
          labels,
          key,
...
function evaluateTypeFromExp(
  ...
  if (value.kind === "TypeConstructorValue" && value.typeConstructor.parameters.length === 0) {
    const dt: DataType = { kind: "DataType", typeConstructor: value.typeConstructor, argTypes: [] }
    return dt
  }
  throw new Error(`expected type expression, got: ${value.kind}`)
}

function computePathOffset(
  mod: Mod,
  baseName: string,
  path: Array<string>,
): number {
...
function makeParamEnv(
  dataType: DataType,
  _structDef: StructDefinition,
): Env {
...
function extractLabelInfo(instr: Instr): {
  name: string
  path: Array<string>
} | null {
