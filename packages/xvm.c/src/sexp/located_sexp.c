...
value_t parse_located_sexps(const char *pathname, const char *string) {
  ...
  while (true) {
    ...
    list_builder_append(&sexps, for_sexp(path, tokens));
  }
...
static value_t for_sexp(value_t path, list_t *tokens) {
  ...
  case QUOTATION_MARK_TOKEN: {
    ...
    list_builder_t elements = list_builder_empty();
    list_builder_append(&elements, head);
    list_builder_append(&elements, for_sexp(path, tokens));
    token_free(token);
    return list_sexp(list_builder_result(&elements), location);
  }
...
static value_t for_elements(value_t path, const char *end, list_t *tokens,
                            struct span_t *out_end_span) {
  list_builder_t sexp = list_builder_empty();
  while (true) {
    ignore_line_comments(tokens);
    if (list_is_empty(tokens)) {
      who_printf("unexpected end of tokens");
    ...
    if (token->kind == BRACKET_END_TOKEN) {
      if (string_equal(token->content, end)) {
        *out_end_span = token->span;
        ...
        token_free(token);
        return list_builder_result(&sexp);
      } else {
        who_printf(
      ...
      list_builder_append(&sexp, for_sexp(path, tokens));
    }
  }
}
