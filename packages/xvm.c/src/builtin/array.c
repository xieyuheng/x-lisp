...
value_t x_array_length(value_t array) {
  return x_int(xarray_length(to_xarray(array)));
}

value_t x_array_is_empty(value_t array) {
  return x_bool(xarray_is_empty(to_xarray(array)));
}

value_t x_array_pop_mut(value_t array) {
  if (xarray_is_empty(to_xarray(array))) {
...
value_t x_array_get(value_t array, value_t index) {
  ...
  size_t length = xarray_length(to_xarray(array));
  if (i >= length) {
...
value_t x_array_reverse_mut(value_t array) {
  xarray_reverse(to_xarray(array));
  return x_void;
}
