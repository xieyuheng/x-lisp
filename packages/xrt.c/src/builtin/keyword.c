...
value_t x_keyword_to_text(value_t keyword) {
  return x_object(make_xtext(keyword_string(to_keyword(keyword))));
}
...
value_t x_keyword_concat(value_t list) {
  ...
  for (int64_t i = 0; i < length; i++) {
    ...
    write_string(buffer, keyword_string(to_keyword(element)));
  }
