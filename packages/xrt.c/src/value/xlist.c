...
const object_class_t xlist_class = {
  ...
  .write_fn = (object_write_fn_t *) write_xlist,
...
static void write_xlist_elements(buffer_t *buffer, object_circle_ctx_t *ctx, const xlist_t *self) {
  for (size_t i = 0; i < array_length(self->elements); i++) {
    write_value_in_ctx(buffer, ctx, xlist_get(self, i));
