...
把 `projects/meta-lisp.js/src/meta/passes/060-LowerMatchPass.ts` 翻译为等价的 `.meta` 文件。

注意：此 pass 依赖 `020-DesugarPass.ts` 中的 `desugar` 和 `create-desugar-state` 函数。
需要把这些函数的实现内联到此文件中（参考 `020-desugar-pass.meta` 中的实现）。

## 目标文件

`projects/meta-lisp.meta/src/meta/passes/060-lower-match-pass.meta`
...
- 需要内联 `create-desugar-state` 和 `desugar` 函数（参考 `020-desugar-pass.meta` 中的实现）
- `exp-free-names`、`exp-occurred-names`、`generate-relative-fresh-name` 在 `projects/meta-lisp.meta/src/meta/exp/` 中
- pattern 相关函数（`is-var-pattern`、`is-data-pattern`、`data-pattern-data-constructor`、`data-pattern-arg-patterns`）在 `projects/meta-lisp.meta/src/meta/pattern/pattern.meta` 中
- 用 `set-union` 替代 `setUnionMany`（可以多次调用）
- `exp-traverse` 在 `exp-traverse.meta` 中
- 查阅 `docs/zh/reference/builtin/index.md` 确认可用函数
- `make-match-clause` 构造 match clause 节点
- `cond-exp` + `cond-clause-t` 构造条件表达式
