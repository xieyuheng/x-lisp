...
cell 是 SSA 值的容器——每个 cell 有唯一的标识（id），并通过传播器网络中的 def-use 关系连接。
...
- 字面量常量（整数、浮点数、布尔值等）通过 `int64`、`float64`、`bool`、`address` 等无输入 cell 的操作产生 cell，而非作为 cell 直接出现。
- cell 的类型由后续的类型推导 pass 决定，而非在定义点显式标注。
...
- `<cell>`（左侧）：输出 cell，指令的计算结果写入此 cell
- `<op>`：操作名 -- 代表了指令的种类
- `<cell>`（右侧）：输入 cell，指令从此 cell 读取运行时值
- `:<key> <attribute>`：属性常量，位于操作形式内部、输入 cell 之后，以 `:` 前缀的 key 区分
...
cell 的类型由后续的类型推导 pass 决定，不在语法中显式标注。
