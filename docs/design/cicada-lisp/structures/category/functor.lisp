...
(define-class functor-t ()
  (dom category-t)
  (cod category-t)
  (map (-> (.object-t dom) (.object-t cod)))
  (fmap (implicit ((x (.object-t dom))
                   (y (.object-t dom)))
          (forall ((f (.morphism-t dom x y)))
            (.morphism-t cod (map x) (map y)))))
  (fmap-preserve-compose
   (implicit ((x (.object-t dom))
              (y (.object-t dom))
              (z (.object-t dom)))
     (forall ((f (.morphism-t dom x y))
              (g (.morphism-t dom y z)))
       (equal-t (.morphism-t cod (map x) (map z))
         (fmap (.compose dom f g))
         (.compose cod (fmap f) (fmap g))))))
  (fmap-preserve-id
   (forall ((x (.object-t dom)))
     (equal-t (.morphism-t cod (map x) (map x))
       (fmap (.id dom x))
       (.id cod (map x))))))
