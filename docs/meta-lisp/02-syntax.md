...
meta-lisp 使用 S-expression 语法。所有代码由**表达式**（expression）和**语句**（statement）组成。
...
| 形式 | 语法 | 语义 | 示例 |
|---|---|---|---|
| 变量 | `name` | 引用局部或全局变量 | `x` `factorial` |
| 限定名 | `mod/name` | 引用模块中导出的名字 | `math/pi` `list/length` |
...
| 函数调用 | `(f arg1 arg2)` | 调用函数 | `(iadd 1 2)` `(f x)` |
