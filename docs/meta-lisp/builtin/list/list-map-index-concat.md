...
```meta-lisp
(all (A B) (-> (list-t A) (-> int-t A (list-t B)) (list-t B)))
```
...
```meta-lisp
(list-map-index-concat (@list 10 20 30) (lambda (i x) (@list i x)))
;; => (@list 0 10 1 20 2 30)

(list-map-index-concat (@list 'a 'b 'c) (lambda (i x) (@list (make-pair i x) (make-pair i x))))
