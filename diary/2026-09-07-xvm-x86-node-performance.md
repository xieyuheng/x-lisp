...
5. **node 编译器本身变快了**：同样关掉 `LocatePass`/`CheckPass`，指令数从
   原实验的 32.4G 降到 23.1G（-29%），这是基线之后 21 个 commit 的优化效果。

6. **类型检查 pass 的成本**：node 打开 `LocatePass`/`CheckPass` 后
   3.888s vs 3.250s（+20%，原实验 7.66s → 6.37s 同样约 +20%），
   新增约 7.3G 条指令。
