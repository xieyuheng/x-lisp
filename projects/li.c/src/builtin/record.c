...
value_t x_make_record(void) {
  return x_object(make_xrecord());
}

value_t x_any_record_p(value_t value) {
  return x_bool(xrecord_p(value));
}

value_t x_record_copy(value_t record) {
  return x_object(xrecord_copy(to_xrecord(record)));
}

value_t x_record_length(value_t record) {
  return x_int(record_length(to_xrecord(record)->attributes));
}

value_t x_record_empty_p(value_t record) {
...
value_t x_record_put(value_t key, value_t value, value_t record) {
...
value_t x_record_delete(value_t key, value_t record) {
  return x_record_delete_mut(key, x_record_copy(record));
}

value_t x_record_append(value_t left, value_t right) {
...
value_t x_record_keys(value_t record) {
...
value_t x_record_values(value_t record) {
...
value_t x_record_entries(value_t record) {
