...
read_mod_body(path_t *path) {
    ...
    if (record_has(mod_bodies, path_raw_string(path))) {
        return (value_t) record_get(mod_bodies, path_raw_string(path));
    }

    file_t *file = open_file_or_fail(path_raw_string(path), "r");
    value_t sexps = parse_sexps(file_read_string(file));
    record_put(mod_bodies, path_raw_string(path), (void *) sexps);
    return sexps;
}
...
basic_load(path_t *path) {
    ...
    if (record_has(loaded_mods, path_raw_string(path))) {
        return record_get(loaded_mods, path_raw_string(path));
    }
